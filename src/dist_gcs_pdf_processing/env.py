...
def load_env_and_credentials():
    ...
    base_dir = (
        os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__f
        ile__)))))
    ...
    print("[DEBUG] .env readable: {os.access(dotenv_path, os.R_OK) if os.path.exists(dotenv_path) else False}")

    if os.path.exists(dotenv_path):
        load_dotenv(dotenv_path=dotenv_path, override=True)
        print("[DEBUG] .env loaded successfully from {dotenv_path}")
    else:
        print("[DEBUG] Could not read .env: [Errno 2] No such file or directory: '{dotenv_path}'")
        print("[DEBUG] CWD: {os.getcwd()}")
        print("[DEBUG] .env loaded at entrypoint: {os.getenv('DOTENV_LOADED')}")
    ...
    if os.path.exists(cwd_dotenv) and not os.path.exists(dotenv_path):
        print("[DEBUG] Loading .env from CWD: {cwd_dotenv}")
    ...
    print("[DEBUG] GOOGLE_APPLICATION_CREDENTIALS: {gcs_creds_path}")
    ...
    if gcs_creds_path:
        ...
        print("[DEBUG] GOOGLE_APPLICATION_CREDENTIALS set to: {gcs_creds_path_fixed}")
        ...
        if os.path.exists(default_gcs_creds):
            ...
            print("[DEBUG] Using default GCS credentials from: {default_gcs_creds}")
        elif os.path.exists(cwd_default_gcs_creds):
            os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = (
                cwd_default_gcs_creds)
            print("[DEBUG] Using default GCS credentials from CWD: {cwd_default_gcs_creds}")
        else:
            print("[DEBUG] No GCS credentials file found. Tried:")
            print("[DEBUG]   - {default_gcs_creds}")
            print("[DEBUG]   - {cwd_default_gcs_creds}")

    # Handle Google Drive credentials
    drive_creds_path = os.getenv('GOOGLE_DRIVE_CREDENTIALS')
    print("[DEBUG] GOOGLE_DRIVE_CREDENTIALS: {drive_creds_path}")

    if drive_creds_path:
        # Always use forward slashes for compatibility
        drive_creds_path_fixed = drive_creds_path.replace('\\', '/')
        os.environ['GOOGLE_DRIVE_CREDENTIALS'] = drive_creds_path_fixed
        print("[DEBUG] GOOGLE_DRIVE_CREDENTIALS set to: {drive_creds_path_fixed}")
        print("[DEBUG] File exists: {os.path.exists(drive_creds_path_fixed)}")
    else:
        ...
        if os.path.exists(default_drive_creds):
            ...
            print("[DEBUG] Using default Drive credentials from: {default_drive_creds}")
        elif os.path.exists(cwd_default_drive_creds):
            os.environ['GOOGLE_DRIVE_CREDENTIALS'] = cwd_default_drive_creds
            print("[DEBUG] Using default Drive credentials from CWD: {cwd_default_drive_creds}")
        else:
            print("[DEBUG] No Drive credentials file found. Tried:")
            print("[DEBUG]   - {default_drive_creds}")
            print("[DEBUG]   - {cwd_default_drive_creds}")

    # Final check
    final_gcs_creds = os.getenv('GOOGLE_APPLICATION_CREDENTIALS')
    final_drive_creds = os.getenv('GOOGLE_DRIVE_CREDENTIALS')
    print("[DEBUG] Final GOOGLE_APPLICATION_CREDENTIALS: {final_gcs_creds}")
    print("[DEBUG] Final GOOGLE_DRIVE_CREDENTIALS: {final_drive_creds}")
    if final_gcs_creds:
        print("[DEBUG] Final GCS credentials file exists: {os.path.exists(final_gcs_creds)}")
    if final_drive_creds:
        print("[DEBUG] Final Drive credentials file exists: {os.path.exists(final_drive_creds)}")
